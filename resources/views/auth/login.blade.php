    ...
    <script>
        document.getElementById('login-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // First attempt with the token Firebase just minted
                let idToken = await cred.user.getIdToken();
                let data = await postSessionToken(idToken);

                // If the server says the token is stale (race condition for new users),
                // force-refresh the token and try once more.
                if (data?.code === 'TOKEN_STALE') {
                    idToken = await cred.user.getIdToken( /* forceRefresh */ true);
                    data = await postSessionToken(idToken);
                }
        ...
        async function postSessionToken(idToken) {
            ...
            // Return data either way so caller can inspect for TOKEN_STALE
            return res.ok ? data : {
                ...data,
